    ...
    async def run():
        ...
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                ...
                assert (await client.get("/api/admin/support/unread-count")).json() == {"tickets": 1, "messages": 2}
                ...
                assert (await client.get("/api/admin/support/unread-count")).json() == {"tickets": 0, "messages": 0}
