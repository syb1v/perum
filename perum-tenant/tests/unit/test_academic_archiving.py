...
def test_subject_and_topics_are_archived_without_deletion_and_restored_explicitly():
    async def run():
        ...
        admin = User(school_id=school.id, role="school_admin", login="archive-admin", password_hash="x"); subject = Subject(school_id=school.id, name="History", in_exchange=True)
        db.add_all([admin, subject]); await db.flush(); topic = Topic(school_id=school.id, subject_id=subject.id, name="Ancient"); db.add(topic); await db.commit()
