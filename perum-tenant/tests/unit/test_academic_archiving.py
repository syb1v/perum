...
from app.modules.journal.service import delete_topic, list_subjects, list_topics, restore_topic
...
def test_subject_and_topics_are_archived_without_deletion_and_restored_explicitly():
    async def run():
        ...
        assert await delete_topic(db, school.id, topic.id, admin) == {"detail": "ok", "is_archived": True}
        assert await restore_topic(db, school.id, topic.id, admin) == {"detail": "ok", "is_archived": False}
