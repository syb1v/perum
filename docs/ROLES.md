...
- **НЕ заходит внутрь школы** (журнал, оценки, ученики/учителя, расписание, аналитика). Это инвариант: внутришкольные эндпоинты возвращают `org_admin` 403 — ровно как `platform_admin` не имеет доступа к данным внутри орг-стеков. Внутреннюю работу школы ведёт её `school_admin`. Если `school_admin` уволился — `org_admin` заводит нового через `/api/schools/{id}/admins` (ядро проксирует во внутренний RPC стека школы: список/добавление/правка/удаление/сброс пароля), а не лезет в данные сам.

> `org_admin` относится к школам так же, как `platform_admin`/ядро относится к организациям: управляет ими и назначает их администраторов, но не оперирует их внутренними данными. Школы изолированы друг от друга (логически, по `school_id`).
