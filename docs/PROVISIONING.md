...
> ⚠️ **v2:** провижинится не орг, а **школа** (silo = школа). `org_admin` поднимает
> школьный стек `school_<slug>_*` через `school_provisioner` (зеркало шагов ниже);
> см. [ARCH_ORG_NODE.md](ARCH_ORG_NODE.md). Текст ниже описывает исходный
> org-провижининг (silo = организация), он переиспользуется как механизм.
