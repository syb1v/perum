...
## Готовность продукта (оценка на №65)

**≈ 86 %** по работающим фичам (взвешенно). **Фазы 5–9 закрыты.** Фаза 9: биллинг-
...
требование «обновление по воздуху по кнопке». Дальше — Фаза 4 (кастомные домены/TLS —
прод-Caddyfile уже есть, нужен `/internal/validate-domain`), Фаза 10 (hardening/RBAC-
матрица/isolation E2E/нагрузка), завершение Фазы 11 (прод-деплой по RUNBOOK), полный
мульти-сервер (школы в БД агента). Нюанс: фронтенд
