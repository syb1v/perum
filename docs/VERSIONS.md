...
## Готовность продукта (оценка на №63)

**≈ 80 %** по работающим фичам (взвешенно). **Фазы 5–8 закрыты.** Плюс выполнена
...
требование «обновление по воздуху по кнопке» (часть Фазы 9). Дальше — Фаза 4 (кастомные
домены/TLS — прод-Caddyfile уже есть), остаток Фазы 9 (биллинг/observability), Фазы 10–11
(hardening/нагрузка/прод), полный мульти-сервер (школы в БД агента). Нюанс: фронтенд
