...
## Готовность продукта (оценка на №29)

**≈ 54 %** по работающим фичам (взвешенно по фазам). **Фаза 6 закрыта полностью**
(журнал, кабинеты ученика и родителя, ДЗ/контрольные, импорт PDF). **Фаза 7
начата** — готовы рейтинги; осталось маркет/квесты/биржа (самая крупная фаза, 16%). Нюанс: фронтенд (легаси,
...
| 7 Геймификация (маркет/биржа/квесты/рейтинги) | 16 | 🔶 ~15% | 2 |
