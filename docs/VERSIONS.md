...
## Готовность продукта (оценка на №27)

**≈ 52 %** по работающим фичам (взвешенно по фазам). **Фаза 6 закрыта полностью**
(журнал, кабинеты ученика и родителя, ДЗ/контрольные, импорт PDF). Следующая —
Фаза 7 (геймификация: маркет/биржа/квесты/рейтинги), самая крупная (16%). Нюанс: фронтенд (легаси,
...
| 7 Геймификация (маркет/биржа/квесты) | 16 | ⬜ | 0 |
