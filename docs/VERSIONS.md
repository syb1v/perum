...
## Готовность продукта (оценка на №43)

**≈ 78 %** по работающим фичам (взвешенно по фазам). **Фазы 5, 6, 7 и 8 закрыты
полностью.** Фаза 5 теперь закрыта **по-настоящему**: добавлено управление
несколькими школами в орг (`org_admin` создаёт школы и переключается между ними;
раньше всё было захардкожено на одну школу — пробел в org-уровневом слое). Фаза 8
завершена апелляциями (после новостей и аналитики). Дальше — Фаза 4 (домены/TLS) и
эксплуатационные Фазы 9–11 (обновления-по-кнопке, observability, деплой). Нюанс:
фронтенд (легаси, ~47k LOC) **скопирован целиком** — почти все экраны уже
присутствуют, поэтому остаток работы — преимущественно **бэкенд + проводка**.
