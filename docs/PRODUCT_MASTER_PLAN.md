...
| Stage F lifecycle matrix | **11/12 = 91.7%** | Automated rows и named CI run `29598407038` прошли; только pilot operator evidence pending |
...
automated matrix и CI закрыты, ожидается one-school pilot evidence.
...
зелёный. Следующий исполнитель выполняет one-school pilot по acceptance matrix
[DYNAMIC_MOBILE_DESCRIPTOR_PLAN.md](DYNAMIC_MOBILE_DESCRIPTOR_PLAN.md) и не
закрывает Stage F без operator evidence.
...
| P0 | Tenant discovery | Частично | готовы public UUID, indexed host/UUID/org-domain discovery, release manifest, authenticated deployment snapshot, Core/Tenant schema parity, atomic Mobile descriptor persistence, API/SemVer preflight, account-scoped capability gating и 24-часовой grace. Request-time traffic lease закрывает старые account/revision/route clients при resume, switch и release transition; automated cold start/resume/switch/upgrade/downgrade/stale snapshot/refresh failure tests и named CI gate зелёные. Остаётся one-school pilot Stage F; детали в `DYNAMIC_MOBILE_DESCRIPTOR_PLAN.md` |
