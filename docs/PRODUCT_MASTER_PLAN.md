...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29700311274` подтвердил cold signatures, isolation, persistence/outage recovery и production Tenant stale-signature fail-closed/recovery по real VERSION timestamp (`0h` unavailable, `48h` ready/clean/EICAR). Exact digests остаются candidate. Остаются operator review, target-node inspect, capacity/fairness pilot и attachment UI; production attachments fail-closed |
