...
disposable recreation persistence и outage/recovery automation добавлены и ждут
зелёного evidence run. Остаются
...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29695347053` подтвердил purpose-built relay/clamd images, empty-volume signatures, isolated updater/backend networks, constrained read-only runtime и clean/EICAR. Gate расширен signature fingerprint persistence, updater outage, clamd fail-closed outage и recreation/EICAR recovery; до зелёного run это automation, не evidence. Остаются operator review, target-node inspect, stale-signature/capacity pilot и attachment UI; production attachments fail-closed |
