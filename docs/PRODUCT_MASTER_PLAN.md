...
charts/exports, drill-down и mutations не включены.

Mobile Student parity получил bounded recent transaction history. Dedicated
student-only `GET /api/student/transactions/recent` возвращает максимум 50 closed
`StudentRecentTransactionOut`, exact authenticated user scope, current/legacy-null
school ownership, deterministic newest-first order и explicit UTC timestamps; legacy
market endpoint не менялся. Mobile показывает последние 30 операций в ливках через
account-scoped memory-only query без disk persistence, full export, purchases или
investment mutations. Для rolling school releases новая schema-v1 capability не
добавлялась: общий Home/screen availability probe скрывает feature после old-Tenant
404 без retry и переиспользует successful bounded data; network/5xx остаются
retryable. Descriptor consumer при fresh sign-in/rediscovery канонизирует missing
known keys в false и отбрасывает unknown future keys, сохраняя strict boolean checks.
...
Mobile school admin/director parity получил social moderation queue/detail и online
...
state защищают sensitive bodies/participant labels. Dismiss, hide reported message и
lock/unlock conversation используют generated action payload, current
`expected_version`, один стабильный `client_action_id` на attempt и не делают
optimistic cache mutation. Success инвалидирует account-scoped list/detail, HTTP 409
терминален для текущего attempt и выполняет authoritative refetch без silent retry;
ambiguous transport retry в пределах screen session повторяет exact payload. Action
form online-only и memory-only: outbox/persistence/push не включены до отдельной
sensitive offline policy. Contract gate = 107 paths, Android/iOS exports и 145 Mobile
tests прошли; live Tenant/device interaction остаётся pilot evidence.
...
empty/offline states. Следующая Mobile-only итерация добавила accessible horizontal
subject bars на фиксированной шкале 1–5 и labelled period cells из уже полученных
child-scoped данных: malformed/out-of-range scalar grade values fail missing, порядок
остаётся authoritative, а пропуски не интерполируются. Text labels, wrapping и
horizontal scroll предусмотрены для доступного чтения и узких экранов без заявления
physical-device/large-font evidence. Contracts/backend/descriptor и exact Parent capability
не менялись. Mobile typecheck, 159 tests, contract gate 108 paths и Android/iOS exports
прошли; exports данных, полная transaction history и любые mutations не включены.
...
states дополнены authoritative class/subject picker, filter-aware persisted caches и
local explicit detail без дополнительных requests. Unfiltered query сохраняет exact
legacy persisted key для offline upgrade; filtered keys изолируют pagination, subject
options ограничены выбранным class. Picker 404 оставляет unfiltered feed, network/5xx
имеют retry; next-page failure требует explicit retry. Android/iOS exports и 153 Mobile
tests готовы; mutations и полноценный offline journal остаются отдельными verticals.
...
Legacy billing sweep приведён к этой политике: hourly job и совместимый
`POST /api/billing/enforce` выполняют только reconciliation, материализуют один
open invoice текущего 30-дневного периода и переводят истёкшую подписку в
`past_due`, не вызывая school provisioner и не меняя lifecycle organization/school.
Canceled subscription не считается платёжной просрочкой. В deployed single-worker
Core payment/reconciliation используют общий keyed lock и покрыты controlled race
test. Перед multi-worker/multi-instance Core обязательны PostgreSQL serialization и
DB invariant для одного open invoice; накопление нескольких пропущенных периодов
остаётся отдельным billing ADR и не реализуется эвристически.
