...
Bounded 5-school fairness/load automation добавлена и ждёт зелёного candidate
evidence run. Остаются
...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29700311274` подтвердил cold signatures, isolation, persistence/outage и production Tenant stale recovery. Gate расширен bounded 5-school fairness scenario: 5 isolated relays, `MAX_CONNECTIONS=2`, burst `6×1 MiB`, concurrent peer scans и exact resource/network inspect; до зелёного run это automation, не capacity evidence. Exact digests остаются candidate. Остаются operator review, target-node inspect/load pilot и attachment UI; production attachments fail-closed |
