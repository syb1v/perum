...
restart persistence остаётся pilot gate. Остаются
...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29695347053` подтвердил purpose-built relay/clamd images, empty-volume signatures, isolated updater/backend networks, constrained read-only runtime и clean/EICAR через relay; exact digests остаются candidate, не approved. Остаются operator review, test-node recreation persistence/outage/stale-signature/capacity pilot и attachment UI; production attachments fail-closed |
