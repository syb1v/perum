...
curated Friends/Homework/preferences/push/social/support-mutation/moderation/teacher-profile/journal-picker/topics read/create/update OpenAPI contracts прошли Core/Tenant full
...
|---:|---|---|---|
| P0 | Shared contracts | Частично | tenant-scoped mobile auth adapter, shared support-role policy, generated Friends/preferences/push/social/requester/admin-support mutation DTO, typed Homework/moderation/teacher-profile/journal picker/work-types/topics read/create/update contracts, Mobile social/support query plans и versioned support-delivery/school-metrics/deployment-snapshot fixtures готовы; Core ingest sanitizer сохраняет только exact aggregates. Остаются topic archive/restore и остальные teacher/journal/query families, дальнейшие telemetry/test-utils и curated OpenAPI contracts |
