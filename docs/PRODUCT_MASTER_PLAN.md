...
candidate scenario и ждёт зелёного stale/recovery evidence run. Остаются
...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29695993596` подтвердил cold signatures, isolation, fingerprint persistence и outage/recreation recovery. Gate теперь запускает production Tenant `ClamAVScanner`: policy `0h` обязан вернуть stale/unavailable до INSTREAM, policy `48h` восстановить ready/clean/EICAR; до зелёного run это automation, не evidence. Exact digests остаются candidate. Остаются operator review, target-node inspect, capacity pilot и attachment UI; production attachments fail-closed |
