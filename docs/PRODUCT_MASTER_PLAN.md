...
cold-volume/freshness/EICAR candidate CI evidence ещё требуется. Остаются
...
| P1 | Media pipeline | Частично | PostgreSQL 15 run `29691375244` подтвердил concurrency/migration; relay candidate run `29693030308` подтвердил constrained Docker runtime и immutable OCI digest. Реализована least-privilege clamd updater topology: отдельная egress network только для freshclam, shared RW/RO signature volume, clamd только на internal backend; candidate CI проверяет empty-volume download, isolation и clean/EICAR через relay. До зелёного run это automation, не evidence. Остаются approved digests, test-node outage/stale-signature/capacity pilot и attachment UI; production attachments fail-closed |
