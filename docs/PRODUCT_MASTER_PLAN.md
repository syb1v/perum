...
это не approval. Clamd candidate блокирует updater egress для internal network. Остаются
...
| P1 | Media pipeline | Частично | private storage/upload foundation, scanner worker hardening и Core drift/bounded relay готовы. PostgreSQL 15 run `29691375244` подтвердил concurrency/migration contract. Relay candidate run `29693030308` подтвердил constrained Docker runtime и immutable OCI digest с SBOM/provenance; candidate не approved. Clamd candidate заблокирован updater egress design: internal scanner network не даёт freshclam доступ к signature mirrors. Остаются approved clamd/relay digests, real Docker inspect/EICAR/network/outage/signature pilot и attachment UI; production attachments fail-closed |
