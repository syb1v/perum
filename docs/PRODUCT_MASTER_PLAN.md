...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29695993596` подтвердил cold signatures, isolation, constrained runtime, signature fingerprint persistence, updater outage continuity, clamd fail-closed outage и recreation clean/EICAR recovery. Exact digests остаются candidate, не approved. Остаются operator review, target-node inspect, stale-signature/capacity pilot и attachment UI; production attachments fail-closed |
