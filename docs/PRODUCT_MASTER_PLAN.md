...
| P1 | Media pipeline | Частично | PostgreSQL run `29691375244` подтвердил concurrency/migration. Candidate run `29700812844` подтвердил cold signatures, isolation, persistence/outage, production Tenant stale recovery и bounded 5-school fairness (`MAX_CONNECTIONS=2`, burst `6×1 MiB`, concurrent peers, exact resource inspect). Run `30375275580` подтвердил fail-closed relay allowlist `VERSION`/`INSTREAM`, `SHUTDOWN` rejection и тот же topology/recovery/fairness contour. Relay accepted admission теперь также bounded active+pending до upstream. Это repository/candidate envelope, не production sizing. Exact digests остаются candidate. Остаются новый candidate run, operator review, target-node inspect/load pilot и attachment UI; production attachments fail-closed |
