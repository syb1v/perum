...
**Готовность ≈ 77 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). Оценка, темп и прогноз сроков — в [VERSIONS.md](VERSIONS.md).
...
- **Фаза 5 — хвосты (✅ готово):** редактор расписания с подгруппами (`PUT /classes/{id}/schedule`, `GET/PUT /teachers/{id}/schedule`), массовое назначение учителей (`PUT /teacher-subjects/sync`), управление пользователями (список/поиск/правка/удаление/баланс/транзакции/массовая регистрация/без класса).
