...
**Осталось:**
1. **Фаза 7 (геймификация) — закрыта.** Рейтинги, маркет (`/api/market/*`), квесты (`/api/student/quests` + `/api/quests/*`), биржа ливок (`/api/exchange/*` — котировки/портфель/вклады/расчёт). Дальше — **Фаза 4** (кастомные домены + TLS, 3%) или **Фаза 8** (новости/аналитика/апелляции, 9%), затем эксплуатационные Фазы 9–11 (обновления-по-кнопке, observability, деплой). Плюс хвосты Фазы 5 (управление пользователями).
