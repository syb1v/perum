...
**Готовность ≈ 95 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). **Фазы 4–10 закрыты + переархитектура v2 «узел организации» (silo=школа, OTA по кнопке).** Осталась Фаза 11 (прод-деплой: CI, реестр образов, реальный сервер). Оценка, темп и прогноз сроков — в [VERSIONS.md](VERSIONS.md).
