...
**Phase 1 и 2 закрыты. Phase 3 (фронт) — в работе.**

Решено: **дизайн и функционал берём из легаси** (`R1dnis/PERUM`). `perum-web` — копия легаси-фронта (Next.js 16, CSS-модули, тёмная тема `#0ea5e9`/`#09090b`, Chart.js, react-query, same-origin `/api`). Уже собирается в нашем стеке. Caddy на каждом хосте сплитит `/api`+`/docs`→бэкенд, остальное→`perum_web`. **Образ собран, но ещё не задеплоен** (нужна адаптация ниже).

Адаптация легаси-фронта (следующие шаги):
1. **Мульти-тенант слой**: hostname → платформа (`admin.*`) vs школа (поддомен). Легаси одно-доменный — надстроить (host-based рендер/гейт).
2. **Сопоставление ролей + login**: новые роли `org_admin/director` ↔ легаси-роутинг (`admin/school_admin/...`); JWT теперь несёт `org_slug`; токен в localStorage (`auth_token`) — выровнять с легаси-`apiClient`.
3. **Платформенный UI** (создание/список орг) — в легаси отсутствует, добавить на `admin.*` (взять из снесённого каркаса).
4. Деплой perum-web → школьные экраны оживают по мере Фаз 5-8 (бэкенд).

Тестируется руками через **Swagger**: control plane `http://admin.perum.local/docs`; tenant `http://acme.perum.local/docs` (войти org_admin'ом — учётка в ответе на создание орг).
...
- **CHANGELOG.md** — при каждом заметном изменении добавлять запись и поднимать версию (`0.0.x`). На русском, человеческим языком, свежее сверху. Текущая версия: `0.0.11`.
