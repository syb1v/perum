...
**Текущая фаза:** **Фазы 0–3, 5–8 закрыты (≈78%).** Осталось: Фаза 4 (домены/TLS), 9 (биллинг/observability/обновления-по-кнопке), 10 (hardening/тесты), 11 (прод-деплой). Сводка — раздел «Что осталось» ниже.
**Последний коммит:** refactor — роль-модель org_admin (управляет школами и их админами, не внутри); см. `git log`.
