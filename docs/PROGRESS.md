...
**Текущая фаза:** **Phase 6 (журнал и оценки) — в работе.** Карта прогресса — в разделе «Статус по фазам» ниже.
**Последний коммит:** feat Phase 6 — teacher journal + grades (см. `git log`).
...
**Готовность ≈ 76 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). Оценка, темп и прогноз сроков — в [VERSIONS.md](VERSIONS.md).
...
| 4 | Кастомные домены + on-demand TLS (`/internal/validate-domain`) | ⬜ |
...
| 9 | Биллинг-заглушки + observability + обновления «по кнопке» (rollout) | ⬜ |
| 10 | Hardening: RBAC-матрица, isolation E2E (cross-org 401, cross-school 404), нагрузка (k6) | ⬜ |
| 11 | Прод-деплой (Ubuntu+Caddy+core+мониторинг) + onboarding пилот-орг | ⬜ |
| — | Миграция данных со старого ПЭРУМ (опционально, за горизонтом) | ⬜ |

**Детализация частично готовых:**
- **Фаза 5 — хвосты (нет):** редактор расписания мышью (PUT с подгруппами), массовое назначение учителей (`/teacher-subjects/sync`), управление пользователями (Регистрация/UserManagement).
...
**Демо-доступ (стенд `acme`):** платформа `admin.perum.local` (`admin`/`admin`); школа `acme.perum.local` — org_admin `ivan@acme.ru` (пароль выдаётся в ответе `POST /api/organizations` при создании орг), демо-учителя (`petrov`/`ivanova`/…), ученики (`student1..24`) и родитель (`parent1`, привязан к двум ученикам 5А) — пароль `test1234`.
...
**Осталось:**
1. **Фаза 7 закрыта** (рейтинги/маркет/квесты/биржа). **Фаза 8 — почти закрыта:** готовы **новости** (`/api/news/*` + `/api/admin/news/*`) и **аналитика** (учитель `GET /api/teacher/analytics/{dashboard,topics,works,students/problem}`; админ `GET /api/admin/dashboard/{overview,performance,deep-economy}` + `POST /api/admin/analytics/track`; модель PageVisit, миграция `tenant_0010`). Осталась по Фазе 8 только **апелляция** на оценки. Дальше — **Фаза 4** (домены+TLS, 3%) и эксплуатационные Фазы 9–11 (обновления-по-кнопке, observability, деплой). Плюс хвосты Фазы 5 (управление пользователями).
