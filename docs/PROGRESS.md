...
**Осталось:**
1. **Фаза 7 (геймификация) — в работе.** Готово: **рейтинги** (`/api/leaderboard/{subject_id}`), **маркет** (`/api/market/*`, ShopItem/UserInventory), **квесты** (`/api/student/quests` + take/claim, Quest/UserQuest). Осталось: **биржа ливок** (`/api/exchange/*` — портфель, котировки предметов, окна торгов — самое сложное в фазе). Контракты — в легаси (`R1dnis/PERUM`), фронт-экраны уже скопированы.
