...
- **Фаза 6 (🔶 ~85%):** есть — журнал учителя (просмотр класса, выставление/правка/удаление оценок), начисление ливок (`points_calculator`), Transaction-история, живой «Обзор школы»; **кабинет ученика** (дневник на неделю с оценками/ДЗ/контрольными, аналитика по периодам, сводка, итоговые оценки); **ДЗ и контрольные** (CRUD учителем + вложения файл/ссылка + скачивание) — `/api/homework`, `/api/control-works`, `/api/attachments/{id}/download`. Нет — импорт оценок из PDF, **кабинет родителя**.
...
**Осталось:**
1. Фаза 6: импорт оценок из PDF (порт парсера), кабинет родителя. **(Готово: кабинет ученика — дневник/оценки/аналитика/итоговые; ДЗ и контрольные — CRUD учителем + вложения.)**
