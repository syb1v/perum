...
**Осталось:**
1. **Фаза 7 (геймификация) — в работе.** Готово: **рейтинги** (`/api/leaderboard/{subject_id}` — ТОП-10 за сезон). Осталось: **маркет** (каталог/покупка за ливки/инвентарь/экипировка — `/api/market/*`), **квесты** (`/api/student/quests` + take/complete/claim, модель Quest/UserQuest), **биржа ливок** (`/api/exchange/*` — портфель, котировки предметов, окна торгов — самое сложное). Контракты — в легаси (`R1dnis/PERUM`), фронт-экраны уже скопированы.
