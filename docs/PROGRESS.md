...
**Осталось:**
1. **Фаза 7 закрыта** (рейтинги/маркет/квесты/биржа). **Фаза 8 — в работе:** готовы **новости** (`/api/news/*` + `/api/admin/news/*`, модели News/NewsLike/NewsRead). Осталось по Фазе 8: **аналитика** (`/api/analytics/dashboard|topics|students/problem` — KPI/проблемные темы/ученики, трекинг посещений), **апелляции** на оценки. Дальше — **Фаза 4** (домены+TLS, 3%) и эксплуатационные Фазы 9–11 (обновления-по-кнопке, observability, деплой). Плюс хвосты Фазы 5 (управление пользователями).
