...
**Готовность ≈ 99 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). **Фазы 4–10 закрыты, Фаза 11 на ~75% (CI/release/прод-env готовы); + v2 «узел организации» (silo=школа, OTA по кнопке); + hardening (шифрование секретов, rate-limit, /metrics, вложения на volume) и продуктовые хвосты (настройки/уведомления/обращения).** Остаётся: выкат на реальный сервер (домен+GHCR+secrets) и опц. полный мульти-сервер. Оценка — в [VERSIONS.md](VERSIONS.md).
