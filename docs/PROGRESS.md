...
**Текущая фаза:** **Фазы 0–3, 5–8 закрыты (≈78%)** в модели «silo = организация». ⚠️ **Принято новое целевое направление — «узел организации» (silo = ШКОЛА, контейнер на школу, OTA-обновления по кнопке): см. [ARCH_ORG_NODE.md](ARCH_ORG_NODE.md).** Это переархитектура Фаз 1–3 (+ сливает «обновления по кнопке» Фазы 9); выполняется поэтапно по плану в ARCH_ORG_NODE. Прежняя сводка «Что осталось» (Фазы 4/9/10/11) ниже — актуальна для старой модели и частично поглощается v2.
**Последний коммит:** docs — зафиксирована архитектура v2 «узел организации» + план; см. `git log`.
...
**Готовность ≈ 98 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). **Фазы 4–10 закрыты, Фаза 11 на ~75% (CI/release/прод-env готовы), + переархитектура v2 «узел организации» (silo=школа, OTA по кнопке).** Остаётся: выкат на реальный сервер (домен+GHCR+secrets) и опц. полный мульти-сервер. Оценка, темп и прогноз сроков — в [VERSIONS.md](VERSIONS.md).
...
| 11 | Прод-деплой: CI (pytest+tsc) + release в GHCR + прод-env/Caddyfile/RUNBOOK ✅; выкат на реальный сервер — остаётся | 🔶 ~75% |
...
| **Мелкие хвосты** | — | Админ-вкладки школы без бэкенда (фронт-заглушки): **уведомления**, **обращения**, **поддержка (почта)**, **настройки школы**, онлайн-пользователи. PDF-импорт оценок не проверен на реальном PDF (нет образца). CLI `perum-core create-org` (опц.). |
| **За горизонтом** | — | Миграция данных со старого ПЭРУМ (опционально). |

> Итого по весам осталось **≈22** из 100 (фазы 4+9+10+11 = 3+6+6+8). Готовность ≈78%.
