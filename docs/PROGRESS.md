...
**Осталось (Фаза 5+):**
1. Редактирование расписания (PUT с подгруппами), массовое назначение учителей (`/teacher-subjects/sync`), расписание учителя, управление пользователями (Регистрация/UserManagement).
2. Личные кабинеты учителя/ученика (их эндпоинты) — частично Фаза 6.
3. Фаза 6: журнал/оценки (Grade, FinalGrade, Homework, …). Фаза 7: геймификация (ливки/маркет/биржа/квесты).
...
- **CHANGELOG.md** — при каждом заметном изменении добавлять запись и поднимать версию (`0.0.x`). На русском, человеческим языком, свежее сверху. Текущая версия: `0.0.16`.
