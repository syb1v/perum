...
- **Фаза 6 (🔶):** есть — журнал учителя (просмотр класса, выставление/правка/удаление оценок), начисление ливок (`points_calculator`), Transaction-история, живой «Обзор школы». Нет — итоговые оценки/аттестация, ДЗ и контрольные (CRUD + вложения), **кабинеты ученика/родителя**, импорт оценок из PDF.
...
**Осталось:**
1. Фаза 6: итоговые оценки/аттестация, ДЗ и контрольные (CRUD + вложения), импорт оценок из PDF (порт парсера), кабинеты ученика/родителя (их эндпоинты).
...
- **CHANGELOG.md** — при каждом заметном изменении добавлять запись и поднимать версию (`0.0.x`). На русском, человеческим языком, свежее сверху. Текущая версия: `0.0.18`.
