...
**Готовность ≈ 40–45 %** (по работающим фичам; фронт ~100% присутствует — скопирован из легаси). Оценка, темп и прогноз сроков — в [VERSIONS.md](VERSIONS.md).
...
- **Фаза 6 (🔶 ~70%):** есть — журнал учителя (просмотр класса, выставление/правка/удаление оценок), начисление ливок (`points_calculator`), Transaction-история, живой «Обзор школы»; **кабинет ученика** (дневник на неделю с оценками/ДЗ/контрольными, аналитика по периодам, сводка, итоговые оценки) — `/api/student/*` + общие `/api/subjects`, `/api/periods`. Нет — ДЗ и контрольные (CRUD учителем + вложения), импорт оценок из PDF, **кабинет родителя**.
...
**Демо-доступ (стенд `acme`):** платформа `admin.perum.local` (`admin`/`admin`); школа `acme.perum.local` — org_admin `ivan@acme.ru` (пароль выдаётся в ответе `POST /api/organizations` при создании орг), демо-учителя (`petrov`/`ivanova`/…) и ученики (`student1..24`) — пароль `test1234`.
...
**Осталось:**
1. Фаза 6: ДЗ и контрольные (CRUD учителем + вложения), импорт оценок из PDF (порт парсера), кабинет родителя. **(Кабинет ученика — готов: дневник/оценки/аналитика/итоговые.)**
