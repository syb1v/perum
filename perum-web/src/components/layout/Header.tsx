...
function NavIcon({ icon }: { icon: string }) {
    switch (icon) {
        ...
        case 'message': return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z" /></svg>;
        default: return null;
    }
}
...
export default function Header() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('social_unread_change', loadUnread);
        return () => { window.clearInterval(interval); window.removeEventListener('focus', loadUnread); window.removeEventListener('social_unread_change', loadUnread); };
    }, [role, pathname]);
                ...
                <nav className={styles.nav}>
                    {navItems.map((item) => (
                        ...
                        >
                            ...
                            {item.href === '/messages' && unreadMessages > 0 && <b className={styles.navBadge}>{unreadMessages > 99 ? '99+' : unreadMessages}</b>}
                        </Link>
                    ))}
                </nav>
