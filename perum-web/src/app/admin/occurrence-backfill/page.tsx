...
type Plan = { plan_token: string; summary: { groups: number; safe_groups: number; ambiguous_groups: number; occurrences_to_create: number; rows_to_link: number }; ambiguities: Ambiguity[] };
...
export default function OccurrenceBackfillPage() {
  ...
  useEffect(() => { void load(); }, []);
  async function apply() { if (!plan || !confirmed) return; setBusy(true); try { const result = await api.post<{ occurrences_created: number; rows_linked: number }>('/admin/maintenance/occurrence-backfill', { plan_token: plan.plan_token }); setMessage(`Создано уроков: ${result.occurrences_created}, связано записей: ${result.rows_linked}`); await load(); } catch (error) { setMessage(error instanceof Error ? `${error.message}. План обновлён.` : 'План изменился'); await load(); } finally { setBusy(false); } }
