...
export default function FriendsPage() {
    ...
    const load = async () => {
        ...
        try {
            ...
            setFriends(friendsData.items); setFriendsCursor(friendsData.next_cursor); setIncoming(incomingData); setOutgoing(outgoingData); setBlocks(blocksData);
    ...
    const loadMoreFriends = async () => {
        if (friendsCursor === null) return;
        setPending('friends-more'); setError('');
        try {
            const data = await api.get<StudentPageDto>(`/social/friends?cursor=${friendsCursor}`);
            setFriends(current => [...current, ...data.items.filter(item => !current.some(existing => existing.id === item.id))]);
            setFriendsCursor(data.next_cursor);
        } catch { setError('Не удалось загрузить следующую страницу'); } finally { setPending(''); }
    };
    ...
    else if (tab === 'friends') content = friends.length ? <>{friends.map(student => card(student, <>{button('Написать', `message-${student.id}`, async () => { const conversation = await api.post<Conversation>('/social/conversations', { student_id: student.id }); router.push(`/messages/${conversation.id}`); })}{button('Удалить', `remove-${student.id}`, () => api.del(`/social/friends/${student.id}`), true)}{button('Заблокировать', `block-${student.id}`, () => api.post('/social/blocks', { student_id: student.id }))}</>))}{friendsCursor !== null && <button className={styles.more} disabled={Boolean(pending)} onClick={() => void loadMoreFriends()}>{pending === 'friends-more' ? 'Загрузка...' : 'Показать ещё'}</button>}</> : <div className={styles.empty}>Здесь появятся ваши друзья</div>;
