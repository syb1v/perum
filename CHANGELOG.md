...
- **Core проксирует орг-домены на ноду.** Исправлена логика Caddy-маршрутизации: при `node_id IS NOT NULL` ядро добавляет маршрут `<org.domain>` → `node.hostname:80`. Раньше орги на нодах пропускались при старте и не создавались при провижининге, что ломало лендинг, когда wildcard DNS указывал на ядро.
  ...
  - **Исправлено создание `OrganizationDomain`** при создании орг через API/UI. Без этой записи on-demand TLS не мог выпустить сертификат для домена организации → `ERR_SSL_PROTOCOL_ERROR`.
...
- **Квесты в демо-данных.** `seed_demo.py` создаёт 3 случайных квеста ("Пятёрочник недели", "Без троек 10 дней", "Математический марафон") с наградами в ливках.
- **Лендинг организации автоматически обновляется при изменении школ.** Раньше лендинг генерировался один раз при создании организации со списком школ "школы появятся здесь"; новые школы не отображались, удалённые/замороженные — оставались в списке. Теперь после создания, удаления, заморозки и разморозки школы лендинг пересоздаётся на ноде с актуальным списком активных школ.
  - `app/services/school_provisioner.py`: добавлен `_refresh_org_landing`; вызывается после успешного провижининга школы (локально и на ноде).
  - `app/routers/schools.py`: `delete_school`, `suspend_school_endpoint`, `unsuspend_school_endpoint` обновляют лендинг после изменения статуса школы.
  - `app/routers/organizations.py`: `_resume_org` обновляет лендинг после разморозки организации.

## [Unreleased] — 2026-06-27

### Прод-сервер: новый IP, скрипты авто-деплоя perum-core

- **Прод-сервер перенесён на `171.22.73.2`.** AGENTS.md обновлён: IP, путь SSH-ключа (`C:\Users\halil\.ssh\id_ed25519`), домен `grsn-panel.ru`. A-записи `@` и `*` уже указывают на новый IP.
- **Скрипт авто-деплоя ядра (bash)** (`deploy/scripts/deploy-core.sh`). Полноценный bash-скрипт для Ubuntu-сервера: удалённый деплой по SSH, локальный деплой, первичная установка (`--setup`: Docker, клон репо, `.env.prod`, предзагрузка docker-socket-proxy), `git pull`, `docker compose pull/up --force-recreate`, `image prune`. **perum_web всегда собирается локально**, потому что `NEXT_PUBLIC_BASE_DOMAIN` вшивается в Next.js-билд — иначе апекс редиректит на `/login` вместо лендинга.
- **PowerShell-скрипт** (`deploy/scripts/deploy-core.ps1`) обновлён под `id_ed25519`.
