...
- Focused Launch API security review выявил P0 global node-agent credential и четыре P1 authorization/transport/privacy риска. Core `0.5.4` выводит distinct per-node HMAC credentials, не отправляет master token и заменяет legacy token при controlled rebootstrap. Tenant `1.1.12` ограничивает analytics exact Teacher assignment, закрывает unpublished attachment IDOR для Student/Parent и отбирает coursework mutation authority после revoke assignment. Plaintext public agent transport остаётся OPEN P1; M4 security gate = NO-GO до authenticated confidential transport и production rotation.
...
- Tenant release повышен до `1.1.8` для immutable production rollout исправления Student diary.
- Исправлен production `500` студенческого дневника: producer теперь формирует строковые ключи дней `"0".."5"`, как требует closed `StudentDiaryOut`, вместо несовместимых integer keys, из-за которых FastAPI отклонял весь ответ на `/api/student/diary`. Web Header после явного fail-closed `503` social rollout прекращает повторный unread polling до перемонтирования, не включая social и не ослабляя серверный guard.
...
- School Admin/Director Mobile получил read-only расписание выбранного учителя из существующего каталога без нового descriptor key: detail endpoint теперь требует exact active teacher текущей школы, возвращает domain `404` для null-school/foreign/inactive/non-teacher записей и закрыт privacy-minimized generated DTO с шестью authoritative днями, deterministic lesson order и nullable subject/class/room display data; nameless teacher получает neutral `Учитель {id}` без login fallback. Existing Web editor compatibility сохранена через schedule/subject/class IDs; Mobile query account+teacher scoped, memory-only, трактует только exact generic router `{detail: "Not Found"}` как unavailable, отдельно показывает stale teacher, повторяет network/408/425/429/5xx и не добавляет время звонков, контакты или mutations.
...
- Native school moderation расширена online authoritative actions для `school_admin|director`: dismiss, hide message и lock/unlock conversation используют generated payload, current optimistic version и стабильный `client_action_id`; double submit блокируется, HTTP 409 выполняет authoritative refetch без silent retry, ambiguous retry в рамках screen session повторяет exact payload, а sensitive evidence/action state не персистятся offline.
...
- Production school provisioning восстановлен на immutable `git-c145d93ad070`: release `1.1.3` зарегистрирован в Core, node fallback/Agent синхронизированы, failed school успешно reprovisioned, Tenant/Web containers healthy и school HTTPS/TLS проверены. Node installer теперь требует явный immutable Tenant image, а Release показывает безопасный Core error body и проверяет returned image/SHA/current identity.
- Agent internal RPC больше не резолвит isolated school container из общей сети: запрос выполняется через Docker exec внутри Tenant app к loopback, при этом method/body/internal tokens передаются только через exec environment. School network isolation сохраняется, HTTP status/data возвращаются без публикации `/internal` через Caddy.
- Node Caddy с `network_mode: host` больше не получает Docker-only hostnames: provisioning, unsuspend и startup resync определяют текущие bridge IP school app, shared Web и organization landing через Docker inspect. Это устраняет `502` после Agent/Caddy/container restart без подключения Caddy к изолированным school networks.
- Remote provisioning теперь передаёт стабильные `Organization.public_id` и `School.public_id`: Agent reconciles shadow metadata по UUID и перепривязывает существующую школу без удаления Tenant DB/volume. Node Web включён в managed Watchtower rollout, чтобы устаревший PWA service worker не блокировал login и не показывал ложный offline после релиза.
- Organization-node Caddy разрешает exact apex только для landing proxy в `org_agent` role; school routes и `admin`/`www` остаются зарезервированы. Это позволяет startup resync восстановить одновременно organization landing и school routes после Caddy restart.
- Core node monitor теперь получает фактическое состояние organization landing от Agent и synchronizes `landing_status` по домену: восстановленный route переводит stale `failed` в `active`, отсутствие контейнера или Caddy route возвращает `failed`, пустое наблюдение ничего не перезаписывает.
- Core Caddy больше не владеет TLS/routes remote schools: startup sync удаляет legacy Core route и делегирует hostname node Caddy, предотвращая конкурирующие ACME renewals. Agent удаляет stale landing routes без контейнеров, считает школы по managed app labels, а node installer использует `pull_policy: missing` для запуска предварительно загруженного image при недоступном GHCR.
- Public organization/school DNS теперь имеет Cloudflare proxy policy: DNS manager создаёт proxied A-records и исправляет существующие DNS-only records во время sweep, чтобы VPN/client traffic не ходил напрямую к нестабильному node origin.
...
- Mobile school admin/director parity получил read-only class directory: existing `GET /admin/classes` закрыт generated `AdminClassesOut`, atomic `school_admin_class_directory` показывает class name, grade, homeroom teacher и student count с exact-role gating и memory-only cache. Contract gate = 105 paths, Mobile = 132 tests, focused Tenant/Core suites and Android/iOS exports passed; roster, student PII, schedules and mutations remain excluded.
- School academic calendar contracts hardened end-to-end: `GET /admin/academic-years` and `GET /admin/school-periods` now return closed generated DTOs, legacy `target_grades` is normalized to nullable integer arrays, Web consumers use generated types, and Mobile exact-role `school_admin_academic_calendar` shows read-only years/periods. Contract gate = 104 paths, Mobile = 130 tests, Web typecheck and Android/iOS exports passed; CRUD mutations and persisted admin calendar data remain excluded.
- Mobile school admin/director parity получил read-only social moderation queue/detail: atomic `school_admin_social_moderation` capability, generated paginated case/evidence DTOs, exact-role gating, cursor-overlap deduplication и memory-only sensitive evidence. Contract gate = 102 paths, Mobile = 127 tests, focused moderation tests and Android/iOS exports passed; actions, outbox, persistence and push remain excluded.
- Mobile school admin/director parity получил read-only school overview: existing dashboard response закрыт generated `AdminDashboardOverviewOut`, Web удалил ручной DTO, atomic `school_admin_overview` capability открывает exact-role Mobile screen с 7/30/90/365-day KPI, classes, grades, attendance, teacher activity и failing students. Sensitive school-wide data остаётся memory-only; contract gate = 102 paths, Mobile = 124 tests, Web typecheck and Android/iOS exports passed.
- Mobile Student parity получил read-only grade analytics: atomic `student_analytics` capability, independently cached generated summary/period analytics, subject averages и nullable period dynamics с loading/error/empty/offline states. Contract parity, focused descriptor suites, Android/iOS exports и 122 Mobile tests прошли; transactions/economy, charts, exports и mutations остаются вне slice.
...
- Parent analytics read family типизирована end-to-end: children, grades summary, period analytics и transactions получили closed generated schemas; parent summary/analytics aliases используют те же DTO, что student owner routes. No-class analytics теперь всегда возвращает `current_period=null`; Web удалил ручные DTO без изменения diary/grades/finals, Promise.all/Abort/rendering или mutations.
- Tenant GET `/api/teacher/my-class` получил закрытый `TeacherHomeroomOut` с assigned/unassigned branches, nullable class, student roster и aggregate stats. Homeroom page удалил ручные network DTO; curated gate фиксирует nested refs, nullable names и active enrollment literal без изменения bulk-balance mutation, selection или refresh lifecycle.
...
- Tenant GET `/api/teacher/analytics/topics` получил закрытые `TeacherAnalyticsTopicsOut`/`TeacherAnalyticsTopicOut` с required class average, topic identity, averages, counts и ratio. Analytics page/report generator удалили неполный ручной DTO; curated gate фиксирует exact non-null surface без изменения query construction, period/report handling, rendering, dashboard/problem-students/works или cache behavior.
...
- Tenant PUT `/api/journal/grades/{grade_id}` получил закрытый `JournalGradeUpdateOut` с authoritative version/grade/points/applied diff/new balance/color receipt. `ViewGradeModal` формирует generated `UpdateGradeRequest` и принимает generated response; curated gate фиксирует оба bindings без изменения PUT optionality, optimistic lock, conflict body, refresh lifecycle или расчётов.
