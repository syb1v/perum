...
- **Проверено на реальной второй ноде** (`150.241.87.91`): школа создана и обновлена ядром на ноде, контейнеры healthy, `https://nodeschool.avari-land.ru/health` → 200 (TLS на платформе → проксирование на ноду → школа). Апдейт `git-6817 → :latest` прошёл с записью в историю `success`.
