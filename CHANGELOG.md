...
- **Проверено на реальной второй ноде** (`150.241.87.91`) вживую: создание, обновление (`git-6817 → :latest`, история `success`), заморозка/разморозка, добавление/сброс пароля админа школы, и выживание маршрута после форс-рестарта ядра — всё через `https://nodeschool.avari-land.ru` (TLS на платформе → проксирование на ноду → школа), HTTP 200.
