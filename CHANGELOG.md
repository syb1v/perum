...
- **Проверено на реальной второй ноде** (`150.241.87.91`) вживую: создание, обновление (`git-6817 → :latest`, история `success`), заморозка/разморозка, добавление/сброс пароля админа школы, выживание маршрута после форс-рестарта ядра, и **полностью автоматическое** создание новой школы (без ручных шагов: org_admin жмёт «Создать школу» → авто-выбор ноды → провижининг → публичный доступ) — всё через `https://<slug>.avari-land.ru`, HTTP 200.
