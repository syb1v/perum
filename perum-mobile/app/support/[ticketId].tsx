...
export default function SupportThreadScreen() {
  ...
  const thread = useInfiniteQuery({
    queryKey: queryKeys.supportThread(account?.id ?? '', ticketId), enabled: Boolean(enabled && account && apiClient && eligible && ticketId), initialPageParam: null as string | null,
  ...
  const refresh = useCallback(() => { if (!enabled || !eligible) return; void detail.refetch(); void thread.refetch(); }, [enabled, eligible, detail.refetch, thread.refetch]);
