...
export default function AdminSupportThreadScreen() {
  ...
  const escalate = async () => {
    const redactedSummary = escalationSummary.trim();
    if (!detail.data || pendingAction || detail.data.status === 'closed' || detail.data.escalation_status !== 'none' || redactedSummary.length < 10) return;
  ...
  return <Screen><KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    {detail.data ? <View style={styles.controls}>
      ...
      {controls('Категория', categories, detail.data.category, value => void applyAction({ kind: 'metadata', field: 'category', value }))}
      {controls('Приоритет', priorities, detail.data.priority, value => void applyAction({ kind: 'metadata', field: 'priority', value }))}
      ...
      {detail.data.status !== 'closed' && detail.data.escalation_status === 'none' ? <View style={styles.controlGroup}><Text style={styles.controlLabel}>Передать организации</Text><Text style={styles.privacy}>Отправится только обезличенное резюме. Не указывайте ФИО, контакты и другие персональные данные.</Text><TextInput value={escalationSummary} onChangeText={setEscalationSummary} maxLength={4000} multiline placeholder="Обезличенно опишите проблему" placeholderTextColor={colors.muted} style={styles.escalationInput} /><Pressable disabled={Boolean(pendingAction) || escalationSummary.trim().length < 10} style={[styles.escalate, (Boolean(pendingAction) || escalationSummary.trim().length < 10) && styles.disabled]} onPress={() => void escalate()}><Text style={styles.escalateText}>Передать на согласование</Text></Pressable></View> : null}
...
const styles = StyleSheet.create({
  privacy: { color: colors.muted, fontSize: 12, lineHeight: 17, marginBottom: 8 },
  escalationInput: { borderColor: colors.border, borderWidth: 1, borderRadius: 12, color: colors.ink, minHeight: 84, padding: 10, textAlignVertical: 'top' },
  escalate: { backgroundColor: colors.primary, borderRadius: 12, marginTop: 8, padding: 10 },
  escalateText: { color: colors.white, fontWeight: '800', textAlign: 'center' },
