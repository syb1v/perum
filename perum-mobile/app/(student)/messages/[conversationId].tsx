...
export default function ThreadScreen() {
  ...
  const historyDays = conversation.data?.history_deletes_at ? Math.max(0, Math.ceil((new Date(conversation.data.history_deletes_at).getTime() - Date.now()) / 86_400_000)) : null;
...
const styles = StyleSheet.create({
  ...
  modalBackdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.7)' }, modalKeyboard: { maxHeight: '92%' }, modal: { backgroundColor: colors.background, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 34 }, modalTitle: { color: colors.ink, fontSize: 22, fontWeight: '800', marginBottom: 20 }, label: { color: colors.ink, fontWeight: '700', marginBottom: 9, marginTop: 4 }, categories: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 18 }, category: { borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9 }, categorySelected: { borderColor: colors.primary, backgroundColor: colors.primary }, categoryText: { color: colors.ink }, categoryTextSelected: { color: colors.white, fontWeight: '700' }, comment: { flex: 0, height: 100, maxHeight: 100, textAlignVertical: 'top', marginBottom: 14 }, notice: { color: colors.muted, fontSize: 13, lineHeight: 18 }, reportError: { color: colors.danger, marginTop: 12, lineHeight: 19 }, success: { color: colors.ink, fontSize: 16, lineHeight: 22, marginBottom: 20 }, actions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 20 }, cancel: { borderColor: colors.border, borderWidth: 1, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12 }, cancelText: { color: colors.ink, fontWeight: '700' },
});
