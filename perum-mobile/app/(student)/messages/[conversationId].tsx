...
export default function ThreadScreen() {
  ...
  }, [account?.id, apiClient, conversation.data?.unread_count, conversationId, latestId, queryClient]);
  const send = async () => { const text = body.trim(); if (!text || text.length > 4000 || !conversation.data?.can_send) return; setBody(''); await sync.enqueue(conversationId, text); };
