...
import { useStudentInventoryQuery } from '../inventory/studentInventoryCore';
...
function AccountHome({ account, apiClient, signOut, busy }: { account: NonNullable<ReturnType<typeof useAuth>['account']>; apiClient: NonNullable<ReturnType<typeof useAuth>['apiClient']>; signOut: () => Promise<void>; busy: boolean }) {
  ...
  const studentInventory = useStudentInventoryQuery(account.id, apiClient, user.role === 'student');
  ...
  return <Screen>
    ...
    {user.role === 'student' && studentInventory.isSuccess ? <Pressable style={styles.primary} onPress={() => router.push('/(student)/inventory' as never)}><Text style={styles.primaryText}>Инвентарь</Text></Pressable> : null}
    ...
    {user.role === 'teacher' && has('teacher_homeroom') ? <Pressable style={styles.primary} onPress={() => router.push('/(teacher)/homeroom' as never)}><Text style={styles.primaryText}>Мой класс</Text></Pressable> : null}
    {user.role === 'teacher' && has('teacher_works') ? <Pressable style={styles.primary} onPress={() => router.push('/(teacher)/works' as never)}><Text style={styles.primaryText}>Работы учеников</Text></Pressable> : null}
    {user.role === 'teacher' && has('teacher_analytics') ? <Pressable style={styles.primary} onPress={() => router.push('/(teacher)/analytics' as never)}><Text style={styles.primaryText}>Аналитика класса</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_overview') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/overview' as never)}><Text style={styles.primaryText}>Обзор школы</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_social_moderation') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/moderation' as never)}><Text style={styles.primaryText}>Модерация общения</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_academic_calendar') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/academic-calendar' as never)}><Text style={styles.primaryText}>Учебный календарь</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_class_directory') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/classes' as never)}><Text style={styles.primaryText}>Классы школы</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_teacher_directory') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/teacher-directory' as never)}><Text style={styles.primaryText}>Учителя школы</Text></Pressable> : null}
    {isSchoolSupportOperator(user.role) && has('school_admin_bell_schedules') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/bell-schedules' as never)}><Text style={styles.primaryText}>Расписание звонков</Text></Pressable> : null}
    {(user.role === 'school_admin' || user.role === 'director') && has('school_admin_work_types') ? <Pressable style={styles.primary} onPress={() => router.push('/(admin)/work-types' as never)}><Text style={styles.primaryText}>Виды работ</Text></Pressable> : null}
...
const styles = StyleSheet.create({
  badge: { alignSelf: 'flex-start', backgroundColor: colors.primarySoft, borderColor: colors.borderHover, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, marginBottom: 28 },
  badgeText: { color: colors.primary, fontSize: 13, fontWeight: '800', letterSpacing: 2 },
  eyebrow: { color: colors.primary, fontSize: 13, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 },
  title: { color: colors.ink, fontSize: 30, fontWeight: '800', lineHeight: 36, marginTop: 8 },
  school: { color: colors.muted, fontSize: 16, marginTop: 8 },
  card: { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: 18, marginTop: 20 },
  cardLabel: { color: colors.muted, fontSize: 13, fontWeight: '600' },
  cardTitle: { color: colors.ink, fontSize: 20, fontWeight: '700', marginTop: 10 },
  ...
  spacer: { flex: 1 },
  primary: { backgroundColor: colors.primary, borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 8 },
