...
export type HomeworkMutation = { id: string; accountId: string; homeworkId: number; clientActionId: string; version: number; status: HomeworkStatus; state: 'pending' | 'sending' | 'retry_wait' | 'failed_permanent'; attempts: number; nextAttemptAt: number; error: string | null; createdAt: number };
