...
export type SupportReadOutboxStore = {
  recover: () => Promise<void>;
  getRunnable: (accountId: string, now: number) => Promise<SupportReadMutation | null>;
  getByAccount: (accountId: string) => Promise<SupportReadMutation[]>;
  getByMessage: (accountId: string, ticketId: string, messageId: string) => Promise<SupportReadMutation | null>;
  put: (mutation: SupportReadMutation) => Promise<void>;
  remove: (accountId: string, id: string) => Promise<void>;
  removeAccount: (accountId: string) => Promise<void>;
};

export type SupportReadResult =
  | { type: 'success' }
...
export function createSupportReadCursorOutboxCore(options: {
  store: SupportReadOutboxStore;
  send: (mutation: SupportReadMutation) => Promise<SupportReadResult>;
  onChange?: (accountId: string) => void | Promise<void>;
  onSuccess?: (accountId: string, ticketId: string) => void | Promise<void>;
  now?: () => number;
  key?: () => string;
  ...
  async function enqueue(accountId: string, ticketId: string, messageId: string) {
    const existing = await options.store.getByMessage(accountId, ticketId, messageId);
    if (existing) return existing;
    const id = key();
    const mutation: SupportReadMutation = { id, accountId, ticketId, messageId, clientActionId: id, state: 'pending', attempts: 0, nextAttemptAt: 0, error: null, createdAt: now() };
    ...
    try {
      ...
      while (mutation && options.canSend?.() !== false) {
        ...
        if (result.type === 'success') {
          ...
          await options.onSuccess?.(accountId, mutation.ticketId);
